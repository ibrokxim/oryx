        ...
        <div class="search">
            <form action="" method="">
                <div class="search-group">
                    <input type="text" value="<?php echo e(request()->input('s')); ?>" name="s"
                    ...
                    <input type="hidden" name="status" value="<?php echo e(request()->input('status',0)); ?>">
                    <input type="hidden" name="city" value="<?php echo e(request()->input('city')); ?>">
                </div>
            </form>
        </div>
    </div>
...
<?php $__env->startSection('content'); ?>
    ...
    <div class="content-wrap">
        ...
        <div class="content">
            ...
            <div class="content-top">
                <div class="content-head_wrap">
                    <form action="<?php echo e(route('parcels.index')); ?>" method="GET" class="parcels-filter" id="filter-form">
                        <input type="text" placeholder="Поиск по треку" name="track" value="<?php echo e(request('track')); ?>">
                        <input type="number" placeholder="ID клиента" name="user_id" value="<?php echo e(request('user_id')); ?>">
                        <button type="submit">
						...
						<button type="button" id="toggle-button" class="btn-toggle btn ">
							<?php echo e(request('parcel') === 'all' ? 'Показать за месяц' : 'Показать за все время'); ?>
                ...
                <div class="buttons-top">
                    ...
                    <form action="">
                        <a class="createuser" href="<?php echo e(route('parcels.create')); ?>" style="margin-right: 10px;">+
                            Добавить </a>
                    </form>
                    <form action="<?php echo e(route('parcels.upload')); ?>" method="POST" enctype="multipart/form-data">
                        <?php echo csrf_field(); ?>
                    ...
                    <form>
                        <a class="createrole"
                           href="<?php echo e(route('parcels.load',array_merge(['status'=>request('status',0)],request()->except('status')))); ?>">
                            <svg width="12" height="15" viewBox="0 0 12 15" fill="none"
                                 xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="buttons-top hide" style="width: 100%;  margin-top: 20px;">
                    <form action="<?php echo e(route('parcels.index')); ?>" id="city">
                        <?php if(request('status',0)>=4 || request('out')==15): ?>
                            <?php echo e(Form::select('in_city', ['Все города','Нур-Султан'=>'Нур-Султан','Алматы'=>'Алматы','др.города'], request('in_city'))); ?>

                        <?php else: ?>
                            <?php echo e(Form::select('city', ['Все города','Алматы'=>'Алматы','Нур-Султан'=>'Нур-Султан и др. города'], request()->input('city'))); ?>

                        <?php endif; ?>
                        <?php echo e(Form::select('out', array_replace(['Все страны'],App\Models\Setting::where([['type',3],['active',1]])->pluck('name','id')->toArray()), request('out'))); ?>

                        <?php if(request('status',0)==4): ?>
                            <?php echo e(Form::select('in_status', ['Все статусы','Не отправлена','Отправлена'], request('in_status'))); ?>

                        <?php endif; ?>
                        <div style="display: inline-block;margin-left: 50px;">
                            <label>c <input type="date" name="ds"
                                            value="<?php echo e(request('ds',now()->subMonth()->format('Y-m-d'))); ?>"></label>
                            <label>по <input type="date" name="de"
                                             value="<?php echo e(request('de',now()->format('Y-m-d'))); ?>"></label>
                        </div>
                        <input type="hidden" name="s" value="<?php echo e(request()->input('s')); ?>">
                        <input type="hidden" name="status" value="<?php echo e(request()->input('status',0)); ?>">
                    </form>
                </div>
            ...
            <div class="parcels-menu">
                <?php $__currentLoopData = __('ui.status'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key=>$status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <li class="<?php echo e(request('status',0)==$key ? 'active' : ''); ?>"><a
                            href="<?php echo e(route('parcels.index',array_merge(['status'=>$key],request()->except('status')))); ?>"><?php echo e($status); ?>

                            (<?php echo e(isset($count[$key])?$count[$key]:0); ?>)</a></li>
                            ...
                            <td class="checks">
                                <div class="default-checkbox">
                                    <input type="checkbox" class="custom-control-input checkbox"
                                           id="ch-only-<?php echo e($item->id); ?>" name="id[]" value="<?php echo e($item->id); ?>">
     ...
     width="20px" height="20px" viewBox="0 0 512 512" style="enable-background:new 0 0 512 512;" xml:space="preserve">
                            ...
                            <td class="nowrap"><?php echo e($item->user_id); ?></td>
                            <td style="max-width: 150px;"><?php echo e($item->track); ?></td>
                            <td class="hide"><?php echo e($item->pid); ?></td>
                            <td class="nowrap"><?php echo e($item->created_at->format('d.m.Y')); ?></td>
                            <td style="max-width: 200px;">
                                <?php if($item->status==4 && request('out')!=15): ?>
                                    <p class="parcels-content"><?php echo e($item->recipient->surname ?? ''); ?> <?php echo e($item->recipient->name ?? ''); ?> <?php echo e($item->recipient->fname ?? ''); ?></p>
                                <?php endif; ?>
                                <?php if($item->status==3 && request('out')!=15): ?>
                                    <p class="parcels-content"><?php echo e($item->recipient->surname ?? ''); ?> <?php echo e($item->recipient->name ?? ''); ?> <?php echo e($item->recipient->fname ?? ''); ?></p>
                                <?php endif; ?>
                                <?php if($item->status==5 && request('out')!=15): ?>
                                    <p class="parcels-content"><?php echo e($item->recipient->surname ?? ''); ?> <?php echo e($item->recipient->name ?? ''); ?> <?php echo e($item->recipient->fname ?? ''); ?></p>
                                <?php endif; ?>
                                <?php if($item->status==0 && request('out')!=15): ?>
                                    <p class="parcels-content"><?php echo e($item->recipient->surname ?? ''); ?> <?php echo e($item->recipient->name ?? ''); ?> <?php echo e($item->recipient->fname ?? ''); ?></p>
                                <?php endif; ?>
                                 <?php if($item->status==1 && request('out')!=15): ?>
                                    <p class="parcels-content"><?php echo e($item->recipient->surname ?? ''); ?> <?php echo e($item->recipient->name ?? ''); ?> <?php echo e($item->recipient->fname ?? ''); ?></p>
                                <?php endif; ?>
                                <?php if($item->status>=4 || request('out')==15): ?>
                                    <div>
                                        <?php $__currentLoopData = ['in_fio','in_city','in_address','in_comment','in_phone']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $element): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <?php if($item->{$element}): ?>
                                                <p><?php echo e($item->{$element}); ?></p>
                                            <?php endif; ?>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    </div>
                                <?php endif; ?>
                            ...
                            <td class="hide"><?php echo e($item->weight); ?></td>
                            <?php if(request('status')==4): ?>
                                <td><?php echo e($item->in_status?'Отправлена':'Не отправлена'); ?></td>
                            <?php endif; ?>
                            <td>
                                <div class="parcel_point">
                                    <img src="/images/point.svg">
                                    <span><span style="width: <?php echo e(request('status')*18); ?>%"></span></span>
                                </div>
                            </td>
                            <td>
                                <div class="parcels-links">
                                    <a class="edit" href="<?php echo e(route('parcels.edit', $item->id)); ?>">
...
<?php echo $__env->make('layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/ibrohim/Backend/php/Laravel/oryx/resources/views/admin/parcels/index.blade.php ENDPATH**/ ?>