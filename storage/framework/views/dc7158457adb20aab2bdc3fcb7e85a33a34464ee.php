<div class="slider-wrap mb-70px">
    <meta name="viewport" content="width=device-width, initial-scale=0.60">
    <div class="slider">

        <div class="slider-item">
            <div class="container flex flex-wrap between align-center">
                <div class="slider-content">
                    <h1 class="slider-head">
                        <b>Быстрая доставка</b> посылок из США в Казахстан
                    </h1>
                    <div class="slider-text">
                        Доставка авиа перелетом из США от 7 дней
                    </div>

                        <a href="/register"><div class="bt btn-red slider-btn">Получите свой адрес в США!</div></a>
                </div>
                <div class="slider-img">
                    <img src=" <?php echo e(asset('assets/images/box0.png')); ?> " class="sliderbox" alt="slider"
                         style="top: 7%;">

                    <div id="scene" data-friction-x="0.1" data-friction-y="0.1" data-scalar-x="25"
                         data-scalar-y="15"
                         style="transform: translate3d(0px, 0px, 0px) rotate(0.0001deg); transform-style: preserve-3d; backface-visibility: hidden; position: relative; pointer-events: none;">
                        <div data-depth="0.10"
                             style="transform: translate3d(-0.6px, -2px, 0px); transform-style: preserve-3d; backface-visibility: hidden; position: relative; display: block; left: 0px; top: 0px;">
                            <img src=" <?php echo e(asset('assets/images/slider.png')); ?> " class="sliderimg" alt="sliderimg">
                        </div>
                    </div>

                </div>

            </div>

        </div>


    </div>


    <div class="container">
        <div class="feat flex flex-wrap between">

            <div class="feat-item wow fadeInUp" col data-wow-offset="25" data-wow-duration="1.5s"
                 data-wow-delay="0.5s"
                 style="visibility: visible; animation-duration: 1.5s; animation-delay: 0.5s; animation-name: fadeInUp;">
                <div class="feat-img">
                    <img src="<?php echo e(asset('assets/images/f1.png')); ?>" alt="slider">
                </div>
                <div class="feat-content">
                    <div class="feat-name">
                        ЭКОНОМНО
                    </div>
                    <div class="feat-text">
                        Наши клиенты не платят налог с продаж
                    </div>
                </div>
            </div>

            <div class="feat-item wow fadeInUp" col data-wow-offset="25" data-wow-duration="1.5s"
                 data-wow-delay="0.8s"
                 style="visibility: visible; animation-duration: 1.5s; animation-delay: 0.8s; animation-name: fadeInUp;">
                <div class="feat-img">
                    <img src=" <?php echo e(asset('assets/images/f2.png')); ?> " alt="f2">
                </div>
                <div class="feat-content">
                    <div class="feat-name">
                        НАДЕЖНО
                    </div>
                    <div class="feat-text">
                        Гарантируем сохранность товаров
                    </div>
                </div>
            </div>

            <div class="feat-item wow fadeInUp" col data-wow-offset="25" data-wow-duration="1.5s"
                 data-wow-delay="1s"
                 style="visibility: visible; animation-duration: 1.5s; animation-delay: 1s; animation-name: fadeInUp;">
                <div class="feat-img">
                    <img src=" <?php echo e(asset('assets/images/f3.png')); ?> " alt="f2">
                </div>
                <div class="feat-content">
                    <div class="feat-name">
                        БЫСТРО
                    </div>
                    <div class="feat-text">
                        Из США в Казахстан от 7 дней
                    </div>
                </div>
            </div>

            <div class="feat-item wow fadeInUp" col data-wow-offset="25" data-wow-duration="1.5s"
                 data-wow-delay="1.2s"
                 style="visibility: visible; animation-duration: 1.5s; animation-delay: 1.2s; animation-name: fadeInUp;">
                <div class="feat-img">
                    <img src=" <?php echo e(asset('assets/images/f4.png')); ?> " alt="f2">
                </div>
                <div class="feat-content">
                    <div class="feat-name">
                        ПРОСТО
                    </div>
                    <div class="feat-text">
                        Покупка в США в 3 простых шага
                    </div>
                </div>
            </div>

        </div>
    </div>


</div>
<?php /**PATH /home/ibrohim/Backend/php/Laravel/oryx/resources/views/blocks/block1.blade.php ENDPATH**/ ?>