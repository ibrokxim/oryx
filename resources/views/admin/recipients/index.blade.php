            	    ...
            	    <div class="search-wrap">
            	        <form action="" method="">
            	            <div class="search-group">
            	                ...
            	                <input value="{{ request()->input('s') }}" type="text" name="s" placeholder="Найти получателя" class="input-search" />
            	            </div>
            	        </form>
            	    </div>

                    <form action="">
                    	<a class="createuser" href="{{route('recipients.create')}}">Добавить получателя +</a>
                    </form>
            	</div>
            </div>


    	    <form action="{{route('recipients.delete')}}" method="POST">
    	    	@csrf
    	        <table class="table tablebordered">
    	            <thead>
    	                <tr>
    	                    ...
    	                    <td>{{ Form::select('status',['Все','Подтвержен','Отклонен','Новый'], request()->input('status',0), ['id'=>'status']) }}</td>
    	                    <td></td>
    	                    <td></td>
    	                </tr>
    	            </thead>
    	            <tbody>
    	                ...
    	                <tr>
    	                    ...
    	                    <td style="width: 90px;">{{ $item->user_id }}</td>
    	                    <td style="width: 200px;">{{ $item->user->surname ?? '' }} {{ $item->user->name ?? '' }} {{ $item->user->fname ?? '' }}</td>

    	                    <td class="hide">{{ $item->surname }} {{ $item->name }} {{ $item->fname }}</td>
    	                    <td>{{$item->created_at->format('d.m.Y / H:i')}}</td>
    	                    <td>
    	                    	@if ($item->confirm)
    	                    		Подтвержен
    	                    	@elseif($item->registration)
    	                    		Отклонен
    	                    	@else
    	                    		<b>Новый</b>
    	                    	@endif
    	                    ...
    	                    <td>
    	                        <a class="usedit" href="{{route('recipients.edit', $item->id)}}">
	...
	<script>
		$(function(){
		    ...
		    $('#status').change(function () {
		    	window.location.href = '{{ route('recipients.index', ['status'=>'']) }}'+$(this).val();
		    });
		});
	</script>
@endsection
