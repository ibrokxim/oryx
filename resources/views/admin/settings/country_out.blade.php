	...
	<div class="content-wrap">
    	...
    	<div class="content content-inner">
    		<form method="POST" enctype="multipart/form-data" action="{{$item->id?route('settings.update', $item->id):route('settings.store', $item->id)}}">
    	        ...
    	        @foreach (['Adress 1','Adress 2','City','State','Zip code','Country','Phone'] as $key=>$name)
    	        	<div class="edit-in">
    		            <p class="in-name">{{ $name }}</p>
    		            <input type="text" name="value[]" value="{{ isset($item->value[$key])?$item->value[$key]:'' }}" class="head-input" />
    		        </div>
    	        @endforeach
    	        <div class="edit-in">
