	...
	<div class="content-wrap">
    	...
    	<div class="content content-inner">
    		<form method="POST" enctype="multipart/form-data" action="{{$item->id?route('settings.update', $item->id):route('settings.store', $item->id)}}">
                ...
                @foreach (App\Models\Setting::where('type',3)->where('active',1)->get() as $country)
                    <div class="edit-in">
                        <p class="in-name">{{ $country->name }} 1 кг.</p>
                        <input type="text" name="value[{{ $country->id }}]" value="{{ old('value', json_decode($item->value,1)[$country->id] ?? '') }}" class="head-input" />
                    </div>
                @endforeach
    	        <div class="edit-in">
