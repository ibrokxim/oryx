...
@section('content')
	...
	<div class="main-content">
		<form method="POST" enctype="multipart/form-data" action="{{$item->id?route('instead.update', $item->id):route('instead.store', $item->id)}}">
			...
			<div class="edit-in-sub">
	            <p class="in-name-sub">Пользователь</p>
	            {{ Form::select('user_id', $recipients, $item->user_id, ['class'=>'edit-input']) }}
	        </div>
	        <div class="edit-in-sub">
	            <p class="in-name-sub">Название товара</p>
	            <input type="text" name="name" value="{{ old('name', $item->name) }}" class="edit-input" />
	        </div>
	        <div class="edit-in-sub">
	            <p class="in-name-sub">Ссылка</p>
	            <input type="text" name="link" value="{{ old('link', $item->link) }}" class="edit-input" />
	        </div>
            <div class="edit-in-sub">
                <p class="in-name-sub">Статус</p>
                {{ Form::select('status', __('ui.istatus'),$item->status, ['class'=>'edit-input']) }}
            </div>
	        <div class="edit-in-sub">
	            <p class="in-name-sub">Комментарий</p>
	            <textarea name="comment" rows="5" class="edit-price comment" style="height: 80px;padding: 10px;" placeholder="Дополнительные данные">{{ old('comment', $item->comment) }}</textarea>
	        </div>

		    <div class="buttons-group">
		    	<a href="{{route('instead.index')}}">Отменить изменения <svg width="16" height="14" viewBox="0 0 16 14" fill="none" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M0.277344 7.03233L8.09108 13.8694V9.90854C10.5498 9.64637 12.5824 10.3457 14.2515 12.0147L15.5196 13.2828V11.4894C15.5196 6.83507 12.9558 4.30712 8.09108 4.07803V0.195312L0.277344 7.03233ZM2.53471 7.03278L6.60657 3.4699V5.54708H7.34942C11.2127 5.54708 13.3297 6.92211 13.8856 9.79361C11.9964 8.52925 9.76382 8.10583 7.2273 8.52859L6.60657 8.63204V10.5957L2.53471 7.03278Z" fill="#515151"/></svg></a>
